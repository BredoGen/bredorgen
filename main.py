...
from handler import AsyncRequestHandler
import utils
import macroses
...
class MainHandler(AsyncRequestHandler):
    ...
    def render_from_string(self, tmpl, callback=None, **kwargs):
        hacks.template.Template(tmpl).generate(callback=callback, **kwargs)
    ...
    def generate_routing(self):
        ...
        for keyword in self.keys:
            ...
            rendered = yield gen.Task(self.render_from_string, route_format, **args)

            page = Page(rendered, keyword)
